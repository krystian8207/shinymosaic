...
page <- function(id, title, content, prev_page, next_page) {
  if (!is.null(prev_page)) {
    prev_page <-  click_button(glue::glue("{title}-{prev_page$id}"), prev_page$title, class = "orange left labeled icon massive", 
                               icon = icon(prev_page$icon), href = route_link(prev_page$id), style = "font-size: 2.5rem;")
  }
  if (!is.null(next_page)) {
    next_page = click_button(glue::glue("{title}-{next_page$id}"), next_page$title, class = "orange right labeled icon massive",
                             icon = icon(next_page$icon), href = route_link(next_page[1]), style = "font-size: 2.5rem;")
  }
